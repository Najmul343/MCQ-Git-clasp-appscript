...
.opt-image { max-width: 100%; max-height: 150px; border-radius: 6px; margin-top: 8px; border: 1px solid #e2e8f0; display: block; cursor: zoom-in; }
...
<div id="lightbox">
  ...
  <div class="lb-img-wrap">
    <div class="lb-loading" id="lbLoading"><div class="lb-spinner"></div> Loading...</div>
    <img class="lb-img" id="lbImg" src="" alt="Full size" />
  </div>
</div>
...
var studentInfo    = {};
var currentQ       = 0;
// KEY FIX: answers is now keyed by ORIGINAL question index (_origIdx), not shuffled position.
// This ensures answers always map correctly to questions regardless of shuffle order.
var answers        = {};  // { origIdx: 'A'|'B'|'C'|'D' }
var visited        = {};  // { shuffledPos: true }
var markedReview   = {};  // { shuffledPos: true }
...
function startTest() {
  ...
  // Screen wake lock — screen off nahi hogi test ke dauran
  requestWakeLock();

  // =====================================================================
  // FIX: Stamp each question with its original index BEFORE shuffling.
  // answers{} will use _origIdx as key so the mapping always stays correct
  // regardless of whether/how questions get shuffled.
  // =====================================================================
...
function showQuestion(index) {
  ...
  var origIdx = q._origIdx; // stable key for answers lookup
  ...
  // Shuffle options display order if enabled.
  // q._so stores the shuffle order so it stays consistent when revisiting.
  ...
  opts.forEach(function(opt, displayPos) {
    ...
    // =====================================================================
    // FIX: Check selected state using origIdx (stable), not shuffled index.
    // =====================================================================
    item.className = 'option-item' + (answers[origIdx] === opt.letter ? ' selected' : '');

    // onclick always passes the real original letter (A/B/C/D) — not display position.
...
// =====================================================================
// FIX: Store answer keyed by ORIGINAL index, not current shuffled position.
// =====================================================================
...
function updatePalette() {
  questions.forEach(function(q, i) {
    ...
    // =====================================================================
    // FIX: Look up answer status using origIdx, not shuffled position i.
    // =====================================================================
...
function updateStats() {
  // Count answered using origIdx keys
...
function submitTest() {
  ...
  testSubmitted = true;
  clearInterval(timerInterval);
  releaseWakeLock();  // Screen lock release — test khatam
  closeModal();

  // =====================================================================
  // FIX: Score using origIdx to look up answers.
  // questions[] is in shuffled order here, but answers{} is keyed by origIdx.
  // This gives the correct score AND ensures the server receives answers
  // keyed by original position — matching what Dashboard.gs expects.
  // =====================================================================
  var score      = 0;
  var totalMarks = 0;
  var correct    = 0;
  ...
  var submitData = {
    ...
    answers:      answers   // already keyed by origIdx — matches server expectation
  };
...
function showResultScreen(score, totalMarks, pct, passed, correct, wrong) {
  ...
  // =====================================================================
  // Answer review — always show questions in the order student saw them
  // (shuffled order), with correct answer highlighted.
  // Use origIdx to look up what the student actually answered.
  // =====================================================================
  ...
  questions.forEach(function(q, i) {
    ...
    // Options displayed in original A/B/C/D order in the review
    var optsHtml = ['A','B','C','D'].map(function(l) {
      ...
      // ADDED: option image display in review
    ...
    // ADDED: explanation image display in review
    ...
    rv.innerHTML =
      ...
      '<div class="review-q-body">' +
        ...
        // ADDED: question image display in review
...
function downloadReportCard() {
  var el = document.getElementById('resultScreen');

  // Convert all Drive images to base64 so html2pdf can render them
    ...
    return new Promise(function(resolve) {
      ...
      fetch(img.src)
        ...
        .then(function(blob) {
          ...
          reader.onloadend = function() { img.src = reader.result; resolve(); };
          reader.readAsDataURL(blob);
        })
        .catch(function() { resolve(); }); // skip if fetch fails
    });
  });
  ...
  Promise.all(promises).then(function() {
    html2pdf().set({
      margin: 10,
      filename: (studentInfo.name || 'Student') + '_ReportCard.pdf',
      html2canvas: { scale: 2 },
...
// ===== LIGHTBOX =====
var lightboxEl, lbImg, lbLoading;
...
function openLightbox(url) {
  ...
  var hdUrl = convertDriveUrlHD(url);
  var fallbackUrl = convertDriveUrl(url);
  ...
  lbImg.onerror = function() {
    if (lbImg.src !== fallbackUrl) {
      lbImg.src = fallbackUrl;
    } else {
...
function setupImageClicks() {
  var qImg = document.getElementById('qImage');
  qImg.style.cursor = 'zoom-in';
  qImg.onclick = function() {
    if (qImg.src && qImg.style.display !== 'none') {
      openLightbox(qImg.src.replace('&sz=w800', '').replace('&sz=w1600', ''));
    }
  };
  
  var grid = document.getElementById('optionsGrid');
  grid.addEventListener('click', function(e) {
    var optImg = e.target.closest ? e.target.closest('.opt-image') : null;
    if (optImg && optImg.tagName === 'IMG') {
      e.stopPropagation();
      openLightbox(optImg.src.replace('&sz=w800', '').replace('&sz=w1600', ''));
    }
  });
}
...
function openMobilePalette() {
  ...
  dst.querySelectorAll('.q-btn').forEach(function(btn) {
    var qi = parseInt(btn.textContent) - 1;
...
// ===== SCREEN WAKE LOCK (screen band nahi hogi test ke dauran) =====
var wakeLock = null;
...
async function requestWakeLock() {
  try {
    if ('wakeLock' in navigator) {
      ...
      // Agar page visible ho jaye toh dobara request karo
    ...
    // Wake Lock support nahi hai toh bhi test chalega
...
// ===== PAGE CLOSE / BACK BUTTON PROTECTION =====
window.addEventListener('beforeunload', function(e) {
  if (testStarted && !testSubmitted) {
    e.preventDefault();
    e.returnValue = 'Test chal raha hai! Bahar jaane se aapka test kho jayega. Kya aap pakka bahar jaana chahte hain?';
